...
export class PolicyService {
  ...
  getPoliciesForUser(userEmail: string): Policy[] {
    return this.allPolicies().filter(policy => 
      policy.assignedTo.includes(userEmail.toLowerCase())
    );
  }

  getPendingCount(userEmail: string): number {
    return this.getPoliciesForUser(userEmail).filter(p => !p.acknowledged).length;
  }

  acknowledgePolicy(policyId: number) {
    this.allPolicies.update(policies => 
      policies.map(p => p.id === policyId ? { ...p, acknowledged: true } : p)
    );
  }
}
